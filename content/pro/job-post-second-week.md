...
includes:
  ...
  - "The post, the screen, and the small test, written for your role"
  - "The screening pipeline: reply speed, the booking step, the reminder, and the no-show rule"
  - "The second-round interview script that filters instead of selling"
  - "Days one to ten with an owner for every item, and the day-ten conversation word for word"
  - "Quiz mode: twelve applicant answers, hungry or not"
---

# The Hiring System, Pro

## Start here: the build prompt

Paste everything below into Claude. Fill in CONTEXT with the real role. It writes the post, the screen, the pipeline, the second-round script, and the ten-day plan with owners.
...
A hundred wrong applicants cost a week of screening. Fewer, right applicants who already know the hard part cost nothing. Sentence one: the role. Sentence two: the hardest true thing. Then a day, what they get, who this is not for, and a first step that takes effort.

## 2. Speed as a filter

The post filters for fit. The reply speed filters for interest. A candidate who applies at nine and hears back at nine-oh-five is talking to an operation that has it together. The booking step happens the same day, the reminder happens the day before, and a no-show gets one more chance, on a date, and then the pipeline moves on.

## 3. The second round

Most interviews try to sell. The second round filters. Three questions that require stories, one live exercise from the actual role (a two-minute pitch, a mock call, a five-line email), and a close that names the gate: training is something you pass, not something you sit through.

## 4. The second week

Week one is adrenaline. Week two is when the hire decides. The four items: a named daily check-in, a same-day debrief after the first bad shift, a visible early win the leader points at in front of the team, and the day-ten conversation. The number to watch is week-two activity.
...
```
DAY TEN. Here are a new hire's answers to "how is it going, what surprised you, what do you need." Tell me whether they have decided to stay, what the leader should do this week, and the one thing to fix in onboarding for the next hire. ANSWERS: [paste]
```
